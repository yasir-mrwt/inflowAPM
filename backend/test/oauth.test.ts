...
const oauthClient = googleOAuthClient as unknown as {
  ...
  verifyIdToken: (options: {
    idToken: string;
    audience: string;
  }) => Promise<{ getPayload: () => StubIdentity | undefined }>;
};
...
async function completeCallback(
  ...
  return agent
    ...
    .query({ code: "stub-google-code", state });
}
...
describe("Google OAuth Flow", { concurrency: false }, () => {
  ...
  // Verify that Google OAuth starts with a random state, required scopes, and a saved Redis session.
  test("GET /google saves random state before redirecting with required scopes", async () => {
    ...
    assert.deepStrictEqual(generatedOptions?.scope, [
      "openid",
      "email",
      "profile",
    ]);
  ...
  // Verify that the callback rejects requests when no saved OAuth session state exists.
  ...
  // Verify that the callback rejects a state value that does not match the saved session state.
  ...
  // Verify that an incomplete Google identity is rejected and the verified OAuth state cannot be reused.
  ...
  // Verify that Google accounts with an unverified email address are rejected.
  ...
  // Verify that a new verified Google identity creates one passwordless user and OAuth account link.
  test("new verified Google identity transactionally creates one passwordless user and link", async () => {
    ...
    const links = await pool.query(
      "SELECT user_id FROM inflowapm.oauth_accounts WHERE provider=$1 AND provider_user_id=$2",
      ["google", "google-new-user"],
    );
    assert.strictEqual(links.rowCount, 1);
    assert.strictEqual(links.rows[0].user_id, oauthUserId);
  });

  // Verify that a returning Google account reuses its existing linked InflowAPM user.
  test("returning linked Google identity reuses the same local user", async () => {
    ...
    assert.strictEqual(response.statusCode, 302);
    const users = await pool.query(
      "SELECT id FROM inflowapm.users WHERE email=$1",
      [googleEmail],
    );
    const links = await pool.query(
      "SELECT id FROM inflowapm.oauth_accounts WHERE provider_user_id=$1",
      ["google-new-user"],
    );
    assert.strictEqual(users.rowCount, 1);
  ...
  // Verify that Google OAuth does not automatically link to an existing local account with the same email.
  test("Google identity is not silently linked to an existing local email", async () => {
    ...
    const links = await pool.query(
      "SELECT id FROM inflowapm.oauth_accounts WHERE provider_user_id=$1",
      ["google-conflict-user"],
    );
    assert.strictEqual(links.rowCount, 0);
  });

  // Verify that the OAuth exchange code is single-use and returns the normal access and refresh tokens.
  test("POST exchange consumes code once and uses normal JWT/refresh-token flow", async () => {
    ...
    const stored = await pool.query(
      "SELECT refresh_token FROM inflowapm.users WHERE id=$1",
      [oauthUserId],
    );
    assert.notStrictEqual(
      stored.rows[0].refresh_token,
      response.body.data.refresh_token,
    );
  ...
  // Verify that the exchange endpoint rejects missing-body, unknown, and expired OAuth exchange codes.
  test("exchange rejects query-only, unknown, and expired codes", async () => {
    const queryOnly = await supertest(app)
      .post("/api/v1/auth/oauth/exchange")
  ...
  // Verify that an OAuth-only user cannot log in with a local password before setting one.
  test("password login safely rejects an OAuth-only account", async () => {
    const response = await supertest(app)
      .post("/api/v1/auth/login")
      .send({ email: googleEmail, password: "SomePassword123" });
    assert.strictEqual(response.statusCode, 401);
    assert.strictEqual(response.body.message, "Invalid email or password");
  });

  // Verify that an OAuth-created user can set a local password through the existing reset-password flow.
  test("password reset sets a password for an OAuth-created account", async () => {
    ...
    const tokenHash = crypto
      ...
      .digest("hex");
    await pool.query(
      "INSERT INTO inflowapm.reset_password_tokens(user_id, token_hash, expires_at) VALUES($1,$2,$3)",
      [oauthUserId, tokenHash, new Date(Date.now() + 15 * 60 * 1000)],
    );
    await supertest(app)
      .post("/api/v1/auth/reset-password")
      .send({ token: rawToken, password: "OAuthPassword123" })
      .expect(200);
    await supertest(app)
      .post("/api/v1/auth/login")
      .send({ email: googleEmail, password: "OAuthPassword123" })
      .expect(200);
  });
