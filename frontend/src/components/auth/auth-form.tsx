...
import { submitAuthIntent, type AuthIntent } from "@/lib/auth-api";
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input id={id} name={name} type={visible ? "text" : "password"} minLength={5} maxLength={100} autoComplete={autoComplete} required aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} className={cn(fieldClass, "pr-11")} />
...
function OAuthOptions() {
  return (
    <div className="grid grid-cols-2 gap-3" aria-label="OAuth providers coming soon">
      {[["Google", "G"], ["GitHub", null]].map(([provider, mark]) => (
        <button key={provider} type="button" disabled aria-disabled="true" title={`${provider} authentication is coming soon`} className="relative inline-flex h-10 items-center justify-center gap-2 rounded-md border border-border bg-surface-inset px-3 text-xs font-medium text-text-muted opacity-60">
          {mark ? <span className="font-semibold" aria-hidden="true">{mark}</span> : <GitHubMark className="size-[15px]" />}
          {provider}
...
const modeCopy: Record<AuthIntent, { eyebrow: string; title: string; description: string; submit: string }> = {
  ...
  register: { eyebrow: "Create workspace", title: "Start monitoring", description: "Create your account now. Project setup follows after authentication is connected.", submit: "Create account" },
  ...
  "reset-password": { eyebrow: "Secure recovery", title: "Choose a new password", description: "Use 5 to 100 characters, matching the current backend password rule.", submit: "Update password" },
};
...
function validate(mode: AuthIntent, data: FormData): FieldErrors {
  ...
  if (mode === "login" || mode === "register" || mode === "reset-password") {
    ...
    if (password.length < 5 || password.length > 100) errors.password = "Password must be 5 to 100 characters.";
    if (mode !== "login" && password !== value("confirmPassword")) errors.confirmPassword = "Passwords must match.";
  }
  return errors;
}

export function AuthForm({ mode }: { mode: AuthIntent }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setStatus(null);
    const nextErrors = validate(mode, new FormData(form));
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length > 0) return;
    setSubmitting(true);
    try {
      const result = await submitAuthIntent(mode);
      setStatus({ kind: result.status === "success" ? "success" : "error", message: result.message });
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-7 space-y-5">
        ...
        {(mode === "login" || mode === "register" || mode === "reset-password") ? <PasswordField id={`${mode}-password`} name="password" label={mode === "reset-password" ? "New password" : "Password"} autoComplete={mode === "login" ? "current-password" : "new-password"} error={errors.password} /> : null}
        {(mode === "register" || mode === "reset-password") ? <PasswordField id={`${mode}-confirm-password`} name="confirmPassword" label="Confirm password" autoComplete="new-password" error={errors.confirmPassword} /> : null}
